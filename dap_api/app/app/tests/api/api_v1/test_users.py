...
def test_update_existing_user_email(
    ...
    user_obj = UserCreateOut(email=random_email(), secret=generate_secret())
    user = crud.user.create(db, obj_in=user_obj)
    r = client.put(f"{settings.API_V1_STR}/collections/users/items/{user.id}", json={"email": "new@mail.com"})
...
def test_update_existing_user_secret(
    ...
    r = client.put(f"{settings.API_V1_STR}/collections/users/items/{user.id}", json={"secret": "new_secret"})
    r_obj = r.json()
    db.refresh(user)
...
def test_update_not_existing_user_email(
    ...
    r = client.put(f"{settings.API_V1_STR}/collections/users/items/{-1}", json={"email": "new@mail.com"})
    assert r.status_code == 404
